...
class BaseCrawler:
    ...
    def __init__(
        ...
        # httpx 0.28 起移除了 proxies=；配置里代理为空时也不传，避免 TypeError
        if self.proxies and any(self.proxies.values() if isinstance(self.proxies, dict) else [self.proxies]):
            try:
                ...
                self.aclient = httpx.AsyncClient(proxy=proxy, **client_kwargs)
        else:
